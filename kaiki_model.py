...
from keras.layers import MaxPooling2D, Activation, Flatten, Conv2DTranspose, \
    Conv2D, Reshape, BatchNormalization, merge, Input
...
def loss(y_true, y_pred):
    ...
    custom_loss = sum((((y_true+1)*127)+1)*mse)

    return custom_loss


def load_kaiki_model(model_file, x=16, y=16, flatten=False):

    assert x % 4 == 0
    assert y % 4 == 0

    model = load_model(model_file, custom_objects={'GomokuConv': GomokuConv,
                                                   'loss': loss})
    layers = model.layers
    try:
        if model.get_layer(name='flatten_shape') == layers[-1]:
            layers.pop()
    except ValueError:
        pass
    try:
        if model.get_layer(name='resize_shape') == layers[-1]:
            layers.pop()
    except ValueError:
        pass
    if flatten:
        output = Flatten(name='flatten_shape')(
            Reshape((x, y), name='resize_shape')(layers[-1].output))
    else:
        output = Reshape((x, y), name='resize_shape')(layers[-1].output)
...
def create_kaiki_model(state_shape):
    ...
    c1 = Conv2D(128, (3, 3), padding='same', data_format='channels_first',
                use_bias=True)(s)
    #b1 = BatchNormalization(axis=1)(c1)
    a1 = Activation(activation='selu')(c1)
    p1 = MaxPooling2D((2, 2), data_format='channels_first')(a1)

    c2 = Conv2D(128, (3, 3), padding='same', data_format='channels_first',
                use_bias=True)(a1)
    ...
    c3 = Conv2D(128, (3, 3), padding='same', data_format='channels_first',
                use_bias=True)(s3)
    ...
    c4 = Conv2D(128, (3, 3), padding='same', data_format='channels_first',
                use_bias=True)(s4)
    ...
    c5 = Conv2D(128, (3, 3), padding='same', data_format='channels_first',
                use_bias=True)(s5)
    #b5 = BatchNormalization(axis=1)(c5)
    a5 = Activation(activation='selu')(c5)
    t5 = Conv2DTranspose(128, (2, 2), strides=(2, 2), padding='same',
                         data_format='channels_first')(a5)

    s6 = merge.concatenate([p4, a5], axis=1)
    c6 = Conv2D(128, (3, 3), padding='same', data_format='channels_first',
                use_bias=True)(s6)
    #b6 = BatchNormalization(axis=1)(c6)
    a6 = Activation(activation='selu')(c6)
    t6 = Conv2DTranspose(128, (2, 2), strides=(2, 2), padding='same',
                         data_format='channels_first')(a6)

    s7 = merge.concatenate([c4, t5, t6], axis=1)
    c7 = Conv2D(128, (3, 3), padding='same', data_format='channels_first',
                use_bias=True)(s7)
    #b7 = BatchNormalization(axis=1)(c7)
    a7 = Activation(activation='selu')(c7)
    t7 = Conv2DTranspose(128, (2, 2), strides=(2, 2), padding='same',
                         data_format='channels_first')(a7)

    s8 = merge.concatenate([t6, a7], axis=1)
    c8 = Conv2D(128, (3, 3), padding='same', data_format='channels_first',
                use_bias=True)(s8)
    #b8 = BatchNormalization(axis=1)(c8)
    a8 = Activation(activation='selu')(c8)
    t8 = Conv2DTranspose(128, (2, 2), strides=(2, 2), padding='same',
                         data_format='channels_first')(a8)

    s9 = merge.concatenate([a2, t7, t8], axis=1)
    c9 = Conv2D(128, (3, 3), padding='same', data_format='channels_first',
                use_bias=True)(s9)
    #b9 = BatchNormalization(axis=1)(c9)
    a9 = Activation(activation='selu')(c9)

    s10 = merge.concatenate([a1, t8, a9], axis=1)
    c10 = Conv2D(128, (3, 3), padding='same', data_format='channels_first',
                 use_bias=True)(s10)
